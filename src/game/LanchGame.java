...
public class LanchGame {
	...
	public static void main(String[] args) {
		...
		HumanPlayer p1 = new HumanPlayer("loki", 'x');
		AIPlayer p2 = new AIPlayer("TAI", 'o');
		Player cp;
		cp = p1; // cp=current player , now p1 is cp

		while (true) {
			System.out.println(cp.name + " turn");
			cp.makeMove();
			TicTacToe.dispBoard();
			if (TicTacToe.checkColWin() || TicTacToe.checkRowWin() || TicTacToe.checkDiag()) {
				System.out.println(cp.name + " has won");
				break; // somebody wins it forcefully come out
			} 
